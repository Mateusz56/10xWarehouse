...
// Watch for changes in selected location and populate form
...
function handleClose() {
  resetForm();
  uiStore.closeEditLocationModal();
}

async function handleSubmit() {
  if (!isValid.value || !selectedLocation.value) return;

  isSubmitting.value = true;
  error.value = '';

  try {
    await warehouseDetailsStore.updateLocation(selectedLocation.value.id, {
...
<template>
  <Dialog :open="uiStore.isEditLocationModalOpen" @update:open="handleClose">
    <DialogContent class="sm:max-w-md">
      <DialogHeader>
        <DialogTitle>Edit Location</DialogTitle>
        <DialogDescription>
          Update the details for this storage location. Location names must be unique within the warehouse.
        </DialogDescription>
      </DialogHeader>
